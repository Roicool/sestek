...
| **Stack Panels** | `src/StackPanels.webflow.tsx` | Ana sayfa "Why global brands are choosing SESTEK" — **üst üste binen panel scrollytelling'i**, `stack-panels.js` v1.4.0 + `stack-panels.css`'in React hali. Her panel (sonuncu hariç) pinlenir; sonraki üstüne kayarken alttaki bekler (Hold), küçülür (0.5), solar, bulanır ve yukarı süzülür; uzun paneller önce içerik kaydırır. Solda Rich Text (H3, italik kısmı vurgu rengi) + **stagger "Request a demo" butonu** (panel başına stil), sağda **sabit oranlı kare video** (Cloudflare Stream mp4 + thumbnail poster). GSAP sitenin global'inden; yoksa, reduced-motion'da, ata elemanda transform varsa ya da **telefonda (≤ 767px)** paneller düz akışta. **Mobil kayma düzeltmesi:** medya kutusu sabit oranlı — video geç gelince panel boyu değişmez, pin ölçümleri bayatlamaz; videolar `muted` attribute + property, `playsinline`, viewport'a yaklaşınca yüklenir, yalnız görünürken oynar, hata olursa poster kalır. Başlık Rich Text (kalın + italik SESTEK'te marka parıltısı), görünüme girince yumuşak belirme. Bkz. [Stack Panels](#stack-panels--designer-propları). |
...
| Prop | Grup | Tip | Varsayılan | Açıklama |
|---|---|---|---|---|
| Title | Header | RichText | `Why **global** brands are choosing *SESTEK*` | Kalın = strong; **italik kısım** marka parıltısı (turkuaz → lila → magenta süpürme) alır |
...
| Brand shine on italic | Header | Boolean | `On` | |
...
| Panel N › Content | Panel 1–6 | RichText | 3 panel dolu | H3 başlık (**italik kısmı vurgu rengi**) + paragraf (kalın = strong). Boş = panel gizli |
| Panel N › Accent colour | Panel 1–6 | Text | tertiary-700 / secondary-500 / primary-500 | Başlıktaki italik kısmın rengi; token ya da renk |
| Panel N › Video URL / Poster URL / Image | Panel 1–6 | Text · Text · Image | Cloudflare linkleri | Poster URL boşsa Image poster olur; video yoksa Image/Poster medya |
| Panel N › Button style | Panel 1–6 | Variant | `Auto` | `Auto` = sırayla White / Brand secondary / Dark · `Brand primary` · `None` |
| Panel N › Media side | Panel 1–6 | Variant | `Right` | `Left` = video solda (tablet/mobilde hep altta) |
