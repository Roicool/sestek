...
| **TTS Demo** | `src/TtsDemo.webflow.tsx` | Text-to-Speech & Voice Cloning demo iframe'i (`tts-cloning-demo.sestek.com`), Webflow'daki iki onaylı embed + boyutlandırma script'inin React hali (gövde `src/DemoEmbed.tsx`, preset `tts`). **Side by side** (/demos): solda eyebrow + başlık + açıklama + 2 stagger buton (sitenin butonları), sağda panel — büyük ekran 593px panel / 728px iframe, kısa masaüstü (≤ 700px yükseklik) 760×728 @ zoom .648 ortalı, telefonda alt alta tam genişlik 840px. **Full width**: yalnız iframe, 960×649 tabanından genişlik ve yüksekliğe göre ölçekli, ortalı. **Hero** (/demos/tts detay sayfası üstü): ortalı eyebrow + `<h1>` + açıklama + 2 buton, altında Full width ölçülerinde panel; H1 sitenin h1 metrikleriyle (`--text--6xl`, tight, medium — ana sayfa Hero ile aynı), "Title (slot)" slot'una `h1-style` sınıflı bir Webflow Heading bırakılırsa onun yerine o basılır (sitenin kendi sınıfı + Localization). iframe mantıksal boyutunu korur, `transform: scale()` ile ölçeklenir (gömülü
belgenin viewport'u her motorda iframe'in CSS genişliği olur; `zoom` iOS
Safari'de farklı davranıp demo'yu bozuyordu); viewport'a yaklaşınca yüklenir, yüklenene dek spinner; dil sayfadan (tr → tr-TR). Bkz. [TTS Demo](#tts-demo--designer-propları). |
| **STT Demo** | `src/SttDemo.webflow.tsx` | Speech Recognition (speech-to-text) demo iframe'i (`sr-demo-performance.sestek.com`, 600×600, kamera + mikrofon) — TTS Demo ile **aynı gövde** (`src/DemoEmbed.tsx`, preset `stt`). Side by side: solda eyebrow + başlık + açıklama + 2 stagger buton, sağda 600px kare panel; kısa masaüstünde zoom .78; **telefonda alt alta, kare, ekran genişliğine ölçekli** (uygulama mantıksal 600px genişliğini korur). Full width: yalnız iframe, ortalı, ölçekli. Hero: TTS ile aynı (ortalı H1 + panel). Dil sayfadan (tr → `lang=tr`). Bkz. [STT Demo](#stt-demo--designer-propları). |
| **Not Found Page** | `src/NotFoundPage.webflow.tsx` | **Tam sayfa 404 şablonu**: cam efektli minimal navbar (logo görseli/metni, 4 link, buton), ortalanmış gövde (büyük soluk "404", eyebrow, başlık, açıklama, 2 buton), ince footer (telif + 4 link). Tamamı **100svh'ye kilitli** (`grid auto/1fr/auto`), hiçbir viewport'ta scroll oluşmaz; kısa ekranda (≤ 560px yükseklik) açıklama gizlenir, rakam küçülür. Arka plan: iki yumuşak marka tonu lekesi + soluk nokta ızgarası (ikisi de kapatılabilir), Light / Dark. Renk ve font sitenin token'larından. SSR açık, sıfır CLS. 404 sayfasına tek başına bırak, Navbar/Footer symbol'ü ekleme. |
...
| Prop | Grup | Tip | Varsayılan | Açıklama |
|---|---|---|---|---|
| Layout | Layout | Variant | `Side by side` | `Full width` = yalnız iframe · `Hero` = ortalı eyebrow + H1 + açıklama + butonlar, altında panel (detay sayfası üstü) |
| Title (slot) | Text | Slot | — | Yalnız Hero: `h1-style` sınıflı Webflow Heading bırakılırsa üretilen `<h1>` yerine geçer |
...
ölçekleme (yükseklik genişliği takip eder) ve ek **Mobile canvas width (px)**
prop'u (varsayılan `0`): telefonda demo uygulamasının kaç px genişlikte
yerleştiğini söyler. iOS Safari, viewport meta'sı olmayan gömülü sayfaları
kendi 980px'lik varsayılan tuvalinde yerleştirir; uygulama solda kalıp sağda
boşluk bırakıyorsa buraya `980` yazılır — ölçek yine uygulamanın 600px'lik
içeriğini kutuya oturtur, artan boş tuval kırpılır (her iki motorda da aynı
sonuç). Varsayılan metin: "Speech to text"
