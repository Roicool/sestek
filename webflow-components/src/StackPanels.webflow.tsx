...
import { StackPanels, DEFAULT_ITEMS, DEFAULT_TITLE, BUTTON_STYLES } from "./StackPanels";

function item(n: number) {
  const g = "Panel " + n;
  const d = DEFAULT_ITEMS[n - 1];
  return {
    ["i" + n + "Content"]: props.RichText({ name: "Content", group: g, defaultValue: d ? d.html : "", tooltip: "Başlık + gövde tek Rich Text: H3 başlık (italik kısmı vurgu rengi alır), altına paragraf; kalın = strong. Boş = panel gizli" }),
    ["i" + n + "Accent"]: props.Text({ name: "Accent colour", group: g, defaultValue: d ? d.accent || "" : "", tooltip: "Başlıktaki italik kısmın rengi: token (--brand-primary--500) ya da renk. Boş = sırayla tertiary / secondary / primary" }),
...
export default declareComponent(StackPanels, {
  ...
  description:
    ...
    "Solda Rich Text + stagger CTA, sağda sabit oranlı kare video (poster'lı, sessiz, yalnız görünürken oynar, yaklaşınca yüklenir). " +
  ...
  group: "Sestek",
  props: {
    title: props.RichText({ name: "Title", group: "Header", defaultValue: DEFAULT_TITLE, tooltip: "Kalın = strong; italik kısım marka parıltısı alır (SESTEK)" }),
    ...
    titleShine: props.Boolean({ name: "Brand shine on italic", group: "Header", defaultValue: true, trueLabel: "On", falseLabel: "Off", tooltip: "İtalik kelimede turkuaz → lila → magenta ışık süpürmesi" }),
