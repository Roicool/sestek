/*!
 * globe-pulse.js v1.0.0
 * Flat globe wireframe with light pulses travelling along the grid lines
 * (Framer "Edge Network" hero visual). The SVG (grid + pulse routes) is
 * injected by this script; all motion is pure CSS (stroke-dashoffset loop
 * defined in css/effects/globe-pulse.css) — zero per-frame JS, no GSAP.
 *
 * Dependencies: css/effects/globe-pulse.css. Nothing else.
 *
 * API:
 *   Sestek.initGlobePulse()  — wire every [data-globe-pulse] on the page
 *
 * DOM (Webflow):
 ...
 *                                         Default: Sestek primary #EC008C.
 *   data-gp-grid="#212121"                grid line colour.
 *   data-gp-speed="1"                     speed multiplier (2 = twice as fast).
 *   data-gp-glow="10"                     glow blur radius (feGaussianBlur).
 *
...
(function (global) {
  ...
  /* Pulse routes stitched from grid segments (verbatim from the reference).
     len = dash + gap = exact route length -> the dashoffset loop is seamless.
     dur is proportional to len so every pulse moves at the same speed. */
  var PULSES = [
    { len: 1397.71, dur: 8.0, delay: 0.0, peak: 1.0, dash: '73.33 1324.38',
      d: 'M 866.03 586.67 L 847.88 513.28 L 822.65 443.71 L 790.78 379.17 L 752.82 320.77 L 709.44 269.53 L 661.39 226.34 L 609.49 191.94 L 554.66 166.93 L 497.83 151.76 L 440.00 146.67 L 440.00 151.76 L 440.00 166.93 L 440.00 191.94 L 440.00 226.34 L 440.00 269.53 L 440.00 320.77 L 440.00 379.17 L 440.00 443.71 L 440.00 513.28 L 440.00 586.67 L 440.00 571.33 L 440.00 556.12 L 440.00 541.03 L 440.00 526.09 L 440.00 511.30 L 440.00 496.67 L 440.00 482.22 L 440.00 467.95 L 440.00 453.87 L 440.00 440.00 L 440.00 423.67 L 440.00 407.67 L 440.00 392.01 L 440.00 376.71 L 440.00 361.79 L 440.00 347.25 L 440.00 333.13 L 440.00 319.42 L 440.00 306.15 L 440.00 293.33' },
    { len: 945.10, dur: 5.4, delay: -1.2, peak: 0.6, dash: '73.33 871.77',
      d: 'M 440.00 146.67 L 382.17 151.76 L 325.34 166.93 L 270.51 191.94 L 218.61 226.34 L 170.56 269.53 L 127.18 320.77 L 89.22 379.17 L 57.35 443.71 L 32.12 513.28 L 13.97 586.67 L 17.11 571.33 L 20.55 556.12 L 24.31 541.03 L 28.37 526.09 L 32.73 511.30 L 37.39 496.67 L 42.35 482.22 L 47.59 467.95 L 53.13 453.87 L 58.95 440.00 L 70.05 440.00 L 81.54 440.00 L 93.41 440.00 L 105.65 440.00 L 118.23 440.00 L 131.16 440.00 L 144.41 440.00 L 157.97 440.00 L 171.83 440.00 L 185.97 440.00' },
    { len: 1625.19, dur: 9.3, delay: -4.4, peak: 0.4, dash: '73.33 1551.86',
  ...
  function build(root) {
    if (root.__gpBuilt) return;
    root.__gpBuilt = true;

    var speed = num(root.getAttribute("data-gp-speed"), 1);
    var glow = num(root.getAttribute("data-gp-glow"), 10);
    var color = cssColor(root.getAttribute("data-gp-color"));
    var grid = cssColor(root.getAttribute("data-gp-grid"));
    if (color) root.style.setProperty("--gp-color", color);
    if (grid) root.style.setProperty("--gp-grid", grid);

    var fid = "gp-glow-" + (++uid);
    var svg = [
      '<svg viewBox="' + VIEWBOX + '" preserveAspectRatio="xMidYMin meet"',
      ' xmlns="http://www.w3.org/2000/svg" role="img"',
      ' aria-label="Globe wireframe with moving light pulses">',
    ...
    GRID.forEach(function (d) {
      svg.push('<path d="' + d + '" vector-effect="non-scaling-stroke"></path>');
    });
    svg.push("</g><g>");
    PULSES.forEach(function (p) {
      svg.push(
        '<path class="gp-pulse" style="--len:' + p.len + "px;--dur:" +
          (p.dur / speed).toFixed(2) + "s;--delay:" + (p.delay / speed).toFixed(2) +
          "s;--peak:" + p.peak + '" stroke-dasharray="' + p.dash +
          '" filter="url(#' + fid + ')" d="' + p.d + '"></path>'
      );
    });
    svg.push("</g></svg>");
  ...
  function initGlobePulse() {
    var roots = document.querySelectorAll("[data-globe-pulse]");
    for (var i = 0; i < roots.length; i++) build(roots[i]);
  }

  global.Sestek = global.Sestek || {};
  global.Sestek.initGlobePulse = initGlobePulse;
})(window);
